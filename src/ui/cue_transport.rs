...
/// (the "skip" action). Sends nothing to the console — it only moves the
/// marker so the next GO fires the cue after the skipped one. The top-bar
/// label re-reads the current cue each frame, so no event is needed.
pub fn fire_skip(cue_manager: &Arc<RwLock<CueManager>>, runtime: &tokio::runtime::Handle) {
    let cue_mgr = cue_manager.clone();
    runtime.spawn(async move {
        cue_mgr.write().await.skip_next();
    });
}
...
pub fn fire_cue_by_id(
    ...
    runtime.spawn(async move {
        let mut mgr = cue_mgr.write().await;
        if let Some(cue) = mgr.fire_cue_id(id) {
            let cue = cue.clone();
            let snapshot = cue
                .snapshot_id
                .and_then(|id| mgr.get_snapshot(&id).cloned());
